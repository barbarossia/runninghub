...
import { ChevronDown, ChevronUp, Play, Pause, CheckCircle2, XCircle, Clock, FileText, Eye, MoreHorizontal } from 'lucide-react';
...
export function ComplexWorkflowContext({
  ...
  const hasPreviousOutputs = (stepNumber: number) => {
    ...
    return prevStep?.status === 'completed' && (prevStep.outputs?.outputs?.length ?? 0) > 0;
  };
  ...
  return (
    <Card className="border-purple-200 bg-purple-50">
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        {isCompleted && (
          <Badge className={cn('bg-green-100 text-green-800', execution.status === 'completed' && 'animate-in fade-in')}>
            Completed
          </Badge>
        )}
      </div>
      ...
      {execution.steps.length > 1 && (
        <div className="mb-4">
          <div className="flex items-center gap-3 mb-2">
            ...
            <span className={cn('text-lg font-bold', getStepStatusColor(execution.status))}>
      ...
      <div className="space-y-3">
        {execution.steps.map((step, index) => {
          ...
          const showViewButton = step.status === 'completed' && (step.outputs?.outputs?.length ?? 0) > 0;
          ...
          return (
            <Card
              ...
              className={cn(
                ...
                isCurrentStep ? 'border-purple-500 bg-white' : 'border-gray-200',
              ...
              >
                <div className="flex items-center gap-3">
                  <div className="flex items-center gap-2">
                    <span className={cn('text-sm font-medium', getStepStatusColor(step.status))}>
                  ...
                  <div className="space-y-3">
                    ...
                    {step.inputs && (
                      <div className="text-sm text-gray-700">
                        <span className="font-medium">Inputs:</span> {step.inputs.length} parameters
                      </div>
                    )}
                    ...
                    {step.outputs?.outputs && (
                      <div className="text-sm text-gray-700">
                        <span className="font-medium">Outputs:</span> {step.outputs.outputs.length} file(s)
                      </div>
                    )}
                    ...
                    <div className="flex gap-2 mt-4">
                      {showViewButton && (
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() => onViewStep && onViewStep(step.stepNumber)}
                        >
                      ...
                      {step.status === 'completed' && onContinue && !isLastStep && (
                        <Button
                          variant="default"
                          size="sm"
                          onClick={() => onContinue && onContinue()}
                          className={cn('animate-in fade-in', step.stepNumber === currentStepIndex + 1 && 'ring-2 ring-purple-400')}
                        >
