...
export function WorkspaceTextEditor({ fileId }: WorkspaceTextEditorProps) {
  ...
  const { uploadedFiles, getTextContent, updateTextContent } = useWorkspaceStore();
  ...
  useEffect(() => {
    if (textContentData) {
      const content = currentLanguage === 'en' ? textContentData.en : textContentData.zh;
  ...
  const handleTranslate = async () => {
    ...
    const result = await translate(currentText, currentLanguage, targetLanguage);
    ...
    if (result.success && result.translatedText) {
      ...
      toast.success(`Translated to ${targetLanguage === 'en' ? 'English' : 'Chinese'}`);
    } else {
  ...
  return (
    <Card className="overflow-hidden">
      <CardContent className="p-0">
        ...
        {/* Text Editor */}
        <div className="p-4">
          <Tabs value={currentLanguage} onValueChange={(v) => setCurrentLanguage(v as 'en' | 'zh')}>
          ...
          <div className="flex gap-2 mt-4">
            <Button
              ...
              disabled={isTranslating || !currentText.trim() || file.status === 'processing'}
              variant="outline"
              size="sm"
              className="flex-1"
            >
            ...
            <Button
              onClick={handleSave}
              disabled={!hasUnsavedChanges || isSaving || file.status === 'processing'}
              size="sm"
              className="flex-1"
            >
              {isSaving ? (
