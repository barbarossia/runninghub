...
import type { Workflow, FileInputAssignment } from "@/types/workspace";
...
export function JobInputEditor({
	...
	const fileInputsByParam = validFileInputs.reduce(
		(acc, assignment) => {
					...
					.map((param) => {
						...
						return (
							<div key={param.id} className="space-y-2">
								...
								{/* Show file errors */}
								{assignedFiles.some(
									(f) => fileValidationErrors[f.filePath],
								) && (
									<div className="text-xs text-red-600 flex items-start gap-1">
										<AlertTriangle className="h-3 w-3 mt-0.5 shrink-0" />
										<span>
											Some files are no longer available. Job may fail.
										</span>
									</div>
								)}
			...
			<div className="p-4 border-t bg-gray-50 space-y-2">
				{hasMissingFiles && (
					<Alert className="py-2">
						...
						<AlertDescription className="text-xs">
							Missing files detected. Consider re-assigning files from the
							gallery.
						</AlertDescription>
					</Alert>
				)}
