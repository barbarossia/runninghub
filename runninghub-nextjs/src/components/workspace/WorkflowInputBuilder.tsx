...
export interface WorkflowInputBuilderProps {
	...
	initialDeleteSourceFiles?: boolean;
	prefillKey?: string;
}
...
export function WorkflowInputBuilder({
	...
	const [uploadingParams, setUploadingParams] = useState<
		Record<string, boolean>
	>({});

	// Initialize local inputs with workflow defaults when workflow changes or prefill updates
	useEffect(() => {
		...
		setLocalInputs(mergedInputs);
		if (typeof initialDeleteSourceFiles === "boolean") {
			setDeleteSourceFiles(initialDeleteSourceFiles);
		} else {
			setDeleteSourceFiles(false);
		}
	}, [workflow.id, prefillKey, initialTextInputs, initialDeleteSourceFiles]);
	...
	const handleAssignSelected = useCallback(
		(paramId: string) => {
			...
			toast.success(
				`${selectedFiles.length} file(s) assigned to ${workflow.inputs.find((p) => p.id === paramId)?.name}`,
			);
		},
	...
	const validationResult = useMemo(() => {
		...
		for (const param of workflow.inputs) {
			...
			if (param.type === "file") {
				...
				if (!assigned) {
					errors.push(
						`Required file parameter "${param.name}" is not assigned`,
					);
				}
			} else {
				...
				{assigned.length > 0 && (
					<div className={cn("grid gap-3 mb-3", getGridClass(assigned.length))}>
						{assigned.map((assignment) => {
							...
							const paddingBottom =
								aspectRatio > 0 ? `${(1 / aspectRatio) * 100}%` : "100%";
							const aspectStyle = {
								paddingBottom,
							};
													...
													onLoad={(e) => {
														...
														if (img.naturalWidth && img.naturalHeight) {
															...
															if (
																...
																console.log(
																	`[Auto-Detect] Updating dimensions for ${assignment.fileName}: ${img.naturalWidth}x${img.naturalHeight}`,
																);
										...
										<div className="absolute inset-0 p-3 flex flex-col justify-between opacity-0 group-hover:opacity-100 transition-opacity">
											...
											<div className="flex items-center justify-between gap-2">
												...
												<Button
													...
													onClick={() =>
														removeFileAssignment(assignment.filePath)
													}
												>
					...
					<div className="flex flex-col items-center justify-center py-6 text-center">
						...
						<div className="flex flex-wrap justify-center gap-2 mb-2">
							<Button
								variant="outline"
								onClick={() =>
									document.getElementById(`file-upload-${param.id}`)?.click()
								}
								disabled={isUploading}
							>
								{isUploading ? (
							...
							<Button
								...
								disabled={
									isUploading ||
									mediaFiles.filter((f) => f.selected).length === 0
								}
							>
				...
				{invalidCount > 0 && (
					<Alert className="mt-3 border-yellow-200 bg-yellow-50">
						...
						<AlertDescription className="text-xs text-yellow-800">
							{invalidCount} file{invalidCount !== 1 ? "s" : ""} filtered out
							(invalid type)
						</AlertDescription>
					</Alert>
				)}
			...
			<div key={param.id} className="border rounded-lg p-4">
				...
				{param.type === "boolean" ? (
					<div className="flex items-center gap-2">
						<Checkbox
							...
							onCheckedChange={(checked) =>
								setLocalInputs((prev) => ({
									...prev,
									[param.id]: checked === true ? "true" : "false",
								}))
							}
						/>
					...
					<Input
						...
						placeholder={
							param.placeholder || `Enter ${param.name.toLowerCase()}`
						}
	...
	const fileParams = useMemo(
		() => workflow.inputs.filter((p) => p.type === "file"),
		[workflow.inputs],
	);
	...
	const handleSwapInputs = useCallback(() => {
		...
		const getMediaFile = (assignment: any): MediaFile => {
			...
			return {
				...
				extension: assignment.fileName.split(".").pop()
					? "." + assignment.fileName.split(".").pop()
					: "",
				selected: false,
		...
		files2.forEach((f) =>
			assignFileToParameter(f.filePath, p1.id, getMediaFile(f)),
		);
		// Move files from p1 to p2
		files1.forEach((f) =>
			assignFileToParameter(f.filePath, p2.id, getMediaFile(f)),
		);
			...
			<div className="space-y-4">
				...
				{workflow.inputs.length === 0 ? (
					<Alert>
						...
						<AlertDescription>
							This workflow has no input parameters defined. Please edit the
							workflow to add parameters.
						</AlertDescription>
					</Alert>
				) : (
				...
				<div className="space-y-3">
					<div className="flex items-start gap-3">
						<Checkbox
							...
							onCheckedChange={(checked) =>
								setDeleteSourceFiles(checked === true)
							}
						/>
						<div className="flex-1">
							<label
								htmlFor="delete-source-files"
							...
							<p className="text-xs text-gray-500 mt-1">
								{deleteSourceFiles ? (
									<span className="text-orange-600">
										Warning: {jobFiles.length} source file
										{jobFiles.length !== 1 ? "s" : ""} will be permanently
										deleted
									</span>
								) : (
