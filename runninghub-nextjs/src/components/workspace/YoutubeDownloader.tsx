...
export function YoutubeDownloader({
	...
	useEffect(() => {
		const savedMode = localStorage.getItem(STORAGE_KEYS.COOKIE_MODE) as
			| "paste"
			| "file"
			| null;
	...
	const handleDownload = async () => {
		...
		if (cookieMode === "paste" && !cookieContent.trim()) {
			toast.warning(
				"No cookies provided. Download will proceed without cookies (public videos only).",
			);
	...
	const hasCookies =
		cookieMode === "paste" ? cookieContent.trim() : cookieFilePath.trim();
			...
			<div className="space-y-2">
				<div className="flex items-center justify-between">
					...
					>
						...
						Cookies (Optional)
						<span className="ml-auto">{showCookies ? "▲" : "▼"}</span>
					</Button>
					{hasCookies && (
						<Button
							variant="ghost"
				...
				{showCookies && (
					<Tabs
						value={cookieMode}
						onValueChange={(v) => handleCookieModeChange(v as "paste" | "file")}
					>
