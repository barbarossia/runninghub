...
export function JobList({ onJobClick, className = '' }: JobListProps) {
  ...
  const filteredJobs = useMemo(() => {
    return jobs.filter((job) => {
      ...
      if (workflowFilter !== 'all' && job.workflowId !== workflowFilter) return false;
      return true;
    });
                      ...
                      <div className="flex items-start gap-3 flex-1 min-w-0">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-1">
                            <h3 className="font-semibold text-sm truncate">{job.workflowName}</h3>
                          ...
                          <div className="flex items-center gap-2 mb-1">
                            <span className="text-xs text-gray-500">
                              {job.fileInputs.length} file{job.fileInputs.length !== 1 ? 's' : ''}
                            </span>
