...
export function WorkflowEditor({
	...
	const updateParameter = useCallback(
		(index: number, updates: Partial<WorkflowInputParameter>) => {
		...
		async (workflowId?: string) => {
			...
			try {
				const response = await fetch(
					`/api/workflow/nodes?workflowId=${targetWorkflowId}`,
				);
				...
				const newParameters: WorkflowInputParameter[] = data.nodes.map(
					(node: CliNode, index: number) => {
				...
				const workflowName = AVAILABLE_WORKFLOWS.find(
					(wf) => wf.id === targetWorkflowId,
				)?.name;
	...
	const handleLoadCustomWorkflowId = useCallback(async () => {
		...
		try {
			...
			const newParameters: WorkflowInputParameter[] = (
			...
			toast.error(
				error instanceof Error
					? error.message
					: "Failed to load custom workflow ID",
			);
		} finally {
	...
	const handleImportJson = useCallback(async () => {
		...
		try {
			...
			if (data.requiresSelection && data.nodes && data.nodes.length > 0) {
				...
				toast.success(
					`Detected ${data.nodes.length} node${data.nodes.length !== 1 ? "s" : ""} from JSON. Please select which to include.`,
				);
			...
			<DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
				...
				<div className="space-y-6 py-4">
					...
					<div className="space-y-4">
						...
						<div>
							<label
								htmlFor="runninghub-workflow-id"
							...
							<div className="space-y-4">
								<div>
									...
									<p className="text-xs text-blue-700">
										Input fields will load automatically when you select a
										workflow
									</p>
								</div>
								...
								<div className="border-t border-blue-200 pt-3">
									...
									<p className="text-xs text-blue-600 mt-1">
										Enter a workflow ID from your RunningHub account to load its
										structure
									</p>
								</div>
								...
								<div className="border-t border-blue-200 pt-3">
									...
									<p className="text-xs text-blue-600 mt-1">
										Import a workflow JSON file exported from your RunningHub
										workspace
									</p>
									{jsonFile && (
								...
								{templateLoaded && (
									<div className="text-xs text-green-700 bg-green-50 p-2 rounded">
										✓ Template loaded successfully! You can now customize the
										fields below.
									</div>
								)}
							</div>
						...
						{parameters.length === 0 ? (
							<Card className="p-8 text-center text-gray-500">
								...
								<p className="text-xs mt-1">
									Select a workflow template above to load parameters.
								</p>
							</Card>
						) : (
				...
				<DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
					...
					<div className="space-y-4 py-4">
						<div className="flex items-center justify-between">
							...
							<div className="flex gap-2">
								<Button
									...
									size="sm"
									onClick={() =>
										setSelectedNodeIds(new Set(detectedNodes.map((n) => n.id)))
									}
						...
						<div className="text-sm text-gray-600 bg-blue-50 p-3 rounded-lg">
							Selected: {selectedNodeIds.size} node
					...
					<DialogFooter>
						...
						>
							{isImportingJson ? (
								...
								<>
									Create Workflow with {selectedNodeIds.size} Node
									{selectedNodeIds.size !== 1 ? "s" : ""}
								</>
							)}
						</Button>
...
function ParameterEditor({
	...
	return (
		<Card
			className={cn("p-4", fromTemplate && "border-blue-200 bg-blue-50/30")}
		>
						...
						<div className="flex items-center gap-2">
							...
							{parameter.type === "boolean" ? (
								<Select
									...
									onValueChange={(value) =>
										onUpdate({ defaultValue: value === "true" })
									}
								>
