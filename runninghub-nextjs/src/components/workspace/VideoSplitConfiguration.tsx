...
interface VideoSplitConfigurationProps {
	onConfigChange?: (
		config: ReturnType<typeof useVideoSplitStore.getState>["splitOptions"],
	) => void;
	disabled?: boolean;
	className?: string;
}
				...
				<div className="space-y-4 pt-4 border-t border-gray-200">
					...
					{splitOptions.mode === "duration" && (
						<div>
							...
							<Input
								...
								onChange={(e) =>
									setSegmentDuration(parseFloat(e.target.value) || 5)
								}
								className="border-gray-300 bg-white text-gray-900 focus:border-indigo-500 focus:ring-indigo-500"
							/>
							<p className="text-xs text-gray-500 mt-1">
								Each segment will be {splitOptions.segmentDuration || 5} seconds
								long
							</p>
						</div>
					)}
					...
					{splitOptions.mode === "count" && (
						<div>
							...
							/>
							<p className="text-xs text-gray-500 mt-1">
								Video will be split into {splitOptions.segmentCount || 2} equal
								parts
							</p>
						</div>
					)}
						...
						<label className="flex items-center gap-3 cursor-pointer group">
							...
							<div className="flex flex-col">
								<span className="text-sm text-gray-700 group-hover:text-gray-900 transition-colors">
									Delete original video after splitting
								</span>
								<span className="text-xs text-gray-500">
									Segments will be saved in the same folder
								</span>
							</div>
						</label>
					</div>
