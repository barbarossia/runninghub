...
export function JobDetail({ jobId, onBack, className = "" }: JobDetailProps) {
	...
	const [complexWorkflowDefinition, setComplexWorkflowDefinition] =
			...
			const loadComplexExecutionDetails = async () => {
				try {
					const response = await fetch(
						`/api/workspace/complex-workflow/execution/${job.complexWorkflowId}`,
					);
					...
					if (data.success && data.execution) {
						...
						if (data.execution.complexWorkflowId) {
							const defResponse = await fetch(
								`/api/workspace/complex-workflow/${data.execution.complexWorkflowId}`,
							);
	...
	const fetchResults = useCallback(async () => {
		...
		try {
			const response = await fetch(
				`${API_ENDPOINTS.WORKSPACE_JOB_RESULTS}?jobId=${jobId}`,
			);
	...
	const handleReRun = async () => {
		...
		try {
			...
			const workflow = useWorkspaceStore
				.getState()
				.workflows.find((w) => w.id === job.workflowId);
			if (!workflow) {
	...
	const handleReQueryStatus = async () => {
		if (!job.runninghubTaskId) {
			toast.error(
				"No RunningHub task ID found for this job. Task may have failed during submission.",
			);
			return;
		}
	...
	// Handle text edit (original content only)
	const handleTextEdit = (filePath: string, value: string) => {
		setEditedText((prev) => ({
			...prev,
			[filePath]: {
				...prev[filePath],
	...
	const handleSaveText = async (filePath: string) => {
		const content = editedText[filePath]?.original;
		...
		<div className={cn("space-y-4 h-full flex flex-col", className)}>
						...
						<Button
							...
							title={
								!job.runninghubTaskId
									? "No RunningHub task ID available (task failed during submission)"
									: undefined
							}
						>
							<RefreshCw
			...
			<div className="flex-1 min-h-0 grid lg:grid-cols-3 gap-4 transition-all duration-300">
				...
				>
						...
						<div className="animate-in fade-in duration-300">
							<ComplexWorkflowContext
								...
								onStopExecution={async () => {
									...
									try {
										await fetch(
											`/api/workspace/complex-workflow/execution/${complexWorkflow.id}/stop`,
											{
												method: "POST",
											},
										);
					...
					<div className="flex-1 min-h-0 flex flex-col gap-4">
												...
												<div className="flex items-center gap-3 mb-3">
													...
													<div className="min-w-0 flex-1">
														...
														<p className="text-xs text-gray-500">
															{formatFileSize(output.fileSize || 0)}
														</p>
													</div>
													<div className="flex gap-2">
						...
						{/* Text Outputs */}
						{hasTextOutputs && (
							<div className="flex flex-col min-h-0 shrink-0 space-y-3">
								<h3 className="text-sm font-medium text-gray-500">
									Text Outputs
								</h3>
								<div className="grid gap-4">
									{resolvedResults?.textOutputs?.map((output, idx) => {
										const originalContent = output.content.original || "";
										const currentVal =
											editedText[output.filePath]?.original ?? originalContent;
										...
										return (
											<Card key={idx} className="flex flex-col overflow-hidden">
												<div className="p-2 border-b bg-gray-50 flex justify-between items-center text-xs font-medium text-gray-500">
													...
													<div className="flex gap-1">
														<Button
															...
															onClick={() => handleSaveText(output.filePath)}
														>
												...
												<Textarea
													value={currentVal}
													onChange={(e) =>
														handleTextEdit(output.filePath, e.target.value)
													}
													className="min-h-[220px] resize-none border-none focus-visible:ring-0 rounded-none p-4 font-mono text-sm"
			...
			{showContinueDialog && complexWorkflow && complexWorkflowDefinition && (
				<ComplexWorkflowContinueDialog
					...
					nextStep={
						complexWorkflowDefinition.steps.find(
							(s) => s.stepNumber === complexWorkflow.currentStep + 1,
						)!
					}
					previousOutputs={
