...
export function MediaSelectionToolbar({
	...
	const duckEncodedCount = useMemo(() => {
		const count = selectedFiles.filter(
			(f) => f.type === "image" && f.isDuckEncoded,
		).length;
		console.log(
			"[MediaSelectionToolbar] Duck-encoded count:",
			count,
			"selectedFiles:",
			selectedFiles.length,
		);
		console.log(
			"[MediaSelectionToolbar] Selected files with isDuckEncoded:",
			selectedFiles.map((f) => ({
				name: f.name,
				type: f.type,
				isDuckEncoded: f.isDuckEncoded,
			})),
		);
		return count;
	}, [selectedFiles]);
	...
	const handleDecode = useCallback(async () => {
		...
		try {
			...
			const duckEncodedFiles = selectedFiles.filter(
				(f) => f.type === "image" && f.isDuckEncoded,
			);
			...
			toast.error(
				error instanceof Error ? error.message : "Failed to decode images",
			);
		} finally {
			setIsDecoding(false);
			setDecodeProgress({ current: 0, total: 0 });
	...
	const openResizeDialog = useCallback(async () => {
		console.log("[openResizeDialog] Called", {
		...
		if (skipResizeDialog) {
			...
			try {
				console.log(
					"[openResizeDialog] Calling onResize with files:",
					selectedFiles,
				);
	...
	const handleResize = useCallback(async () => {
		...
		try {
			...
			toast.error(
				error instanceof Error ? error.message : "Failed to resize files",
			);
		} finally {
			setIsResizing(false);
		}
	...
	const handleCaption = useCallback(async () => {
		...
		(async () => {
			try {
				...
				toast.error(
					error instanceof Error ? error.message : "Failed to caption media",
				);
			}
		})();
	...
	console.log(
		"[MediaSelectionToolbar] Decode button should show:",
						...
						<DialogFooter>
							...
							>
								{isDecoding ? (
									...
									`Decode ${duckEncodedCount} Images`
								)}
							</Button>
						</DialogFooter>
					</DialogContent>
			...
			{onResize && (
				<Dialog open={showResizeDialog} onOpenChange={setShowResizeDialog}>
					<DialogContent>
						<DialogHeader>
							<DialogTitle>
								Resize {selectedCount === 1 ? "File" : `${selectedCount} Files`}
							</DialogTitle>
							<DialogDescription>
								Scale images and videos by specifying the longest edge. The
						...
						<div className="py-4 space-y-4">
							<div>
								...
								<p className="text-xs text-muted-foreground mt-2">
									Example: A 720×1280 video with longest edge 768 will be
									resized to 432×768
								</p>
							</div>
