...
export function JobSeriesNav({
  ...
  const getStatusIcon = (status: Job['status']) => {
    ...
    switch (status) {
      case 'pending': return <Clock className={iconClassName} />;
      case 'running': return <Loader2 className={cn(iconClassName, 'animate-spin')} />;
      case 'completed': return <CheckCircle2 className={iconClassName} />;
      case 'failed': return <XCircle className={iconClassName} />;
    }
  };
