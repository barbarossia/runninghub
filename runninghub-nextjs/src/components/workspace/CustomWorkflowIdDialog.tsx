...
export function CustomWorkflowIdDialog({
	...
	useEffect(() => {
		if (open && step === "enter") {
			...
			setWorkflowName(
				`Custom Workflow ${new Date(timestamp).toLocaleTimeString()}`,
			);
		}
	}, [open, step]);
		...
		try {
			...
			toast.error(
				error instanceof Error
					? error.message
					: "Failed to fetch workflow. Please check the workflow ID.",
			);
		} finally {
			setIsLoading(false);
		}
	...
	const handleSaveToEnvLocal = async () => {
		...
		try {
			...
			toast.error(
				error instanceof Error
					? error.message
					: "Failed to save workflow to .env.local",
			);
		} finally {
			setIsSaving(false);
		}
				...
				{step === "review" && (
					<div className="space-y-4 py-4">
						<Alert>
							...
							<AlertDescription>
								Found workflow with {fetchedWorkflow?.inputs?.length || 0} input
								parameter(s)
							</AlertDescription>
						</Alert>
