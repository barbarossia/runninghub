...
const RESIZE_MODES = [
  ...
  { value: 'shortest', label: 'Shortest Edge', description: 'Fit shortest side' },
];
...
export function ResizeConfiguration({
  ...
  const [localHeight, setLocalHeight] = useState(resizeHeight?.toString() || '');
  const [localSuffix, setLocalSuffix] = useState(resizeOutputSuffix || '_resized');

  useEffect(() => {
    onConfigChange?.(useCaptionStore.getState());
  }, [resizeMode, resizeWidth, resizeHeight, resizePercentage, resizeAspectRatioStrategy, resizeOutputSuffix, resizeDeleteOriginal, onConfigChange]);

  const handleWidthChange = (value: string) => {
    setLocalWidth(value);
    setResizeDimensions({ width: parseInt(value) || undefined, height: resizeHeight });
  };

  const handleHeightChange = (value: string) => {
    setLocalHeight(value);
    setResizeDimensions({ width: resizeWidth, height: parseInt(value) || undefined });
  };
  ...
  const getSubtitle = () => {
    ...
    if (resizeMode === 'dimensions') return `${localWidth || '?'} x ${localHeight || '?'}px`;
  ...
  return (
    ...
    >
      <div className="space-y-5">
          ...
          <div className="grid grid-cols-2 gap-2">
            {RESIZE_MODES.map((mode) => (
              ...
              >
                ...
                <span className={cn('text-xs', resizeMode === mode.value ? 'text-blue-100' : 'text-gray-500')}>
                  {mode.description}
                </span>
              </button>
            ))}
        ...
        {(resizeMode === 'longest' || resizeMode === 'shortest') && (
          <div className="space-y-3 pt-3 border-t border-gray-200">
            ...
            <Input
              ...
              onChange={(e) => setResizeDimensions({ width: parseInt(e.target.value) })}
              disabled={disabled}
              placeholder="1920"
              className="w-32"
            />
            ...
            <div className="grid grid-cols-3 gap-2">
              {ASPECT_RATIO_STRATEGIES.map((strategy) => (
                <button
                  key={strategy.value}
                  type="button"
                  onClick={() => setResizeAspectRatioStrategy(strategy.value as any)}
                  disabled={disabled}
                  className={cn(
                ...
                >
                  ...
                  <span className={cn('text-[10px]', resizeAspectRatioStrategy === strategy.value ? 'text-blue-100' : 'text-gray-500')}>
        ...
        {/* Output Suffix */}
        <div className="space-y-3 pt-3 border-t border-gray-200">
          <label className="block text-xs font-medium text-gray-700 mb-1">Output Suffix</label>
          <Input
            type="text"
            value={localSuffix}
        ...
        <label className="flex items-center gap-3 cursor-pointer pt-3 border-t border-gray-200">
          ...
          <div className="flex flex-col">
            <span className="text-sm text-gray-700 font-medium">Delete original after resize</span>
