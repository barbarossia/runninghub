...
interface RenameConfigurationProps {
  onConfigChange?: (config: ReturnType<typeof useCaptionStore.getState>) => void;
  disabled?: boolean;
  className?: string;
}

const RENAME_PATTERNS = [
  { value: 'prefix-sequence', label: 'Prefix + Number', description: 'photo001.jpg, photo002.jpg' },
  { value: 'suffix-sequence', label: 'Suffix + Number', description: 'image-001.jpg, image-002.jpg' },
  { value: 'custom-template', label: 'Custom Template', description: 'Use variables' },
];
...
export function RenameConfiguration({
  ...
  }, [renamePattern, renamePrefix, renameSuffix, renameStartNumber, renamePadding, renameTemplate, renamePreserveExtension, onConfigChange]);
  ...
  const getPreview = () => {
    ...
    if (renamePattern === 'prefix-sequence') {
      ...
      return localTemplate
        ?.replace('{index}', num)
        .replace(/\{index:(\d+)d\}/, (_, w) => String(renameStartNumber).padStart(parseInt(w), '0'))
    ...
    >
      <div className="space-y-5">
        ...
        <div className="space-y-3">
          <div className="text-sm font-medium text-gray-700">Rename Pattern</div>
          <div className="grid grid-cols-1 gap-2">
            {RENAME_PATTERNS.map((pattern) => (
              ...
              >
                ...
                <span className={cn('text-xs', renamePattern === pattern.value ? 'text-green-100' : 'text-gray-500')}>
                  {pattern.description}
                </span>
              </button>
            ))}
        ...
        {renamePattern === 'suffix-sequence' && (
          <div className="space-y-3 pt-3 border-t border-gray-200">
            <label className="block text-xs font-medium text-gray-700 mb-1">Separator/Suffix</label>
            <Input
              type="text"
              value={localSuffix}
              onChange={(e) => {
        ...
        {renamePattern === 'custom-template' && (
          <div className="space-y-3 pt-3 border-t border-gray-200">
            <label className="block text-xs font-medium text-gray-700 mb-1">Custom Template</label>
            <Input
              type="text"
              value={localTemplate}
              onChange={(e) => {
        ...
        {/* Number Settings */}
        {(renamePattern === 'prefix-sequence' || renamePattern === 'suffix-sequence') && (
          <div className="space-y-3 pt-3 border-t border-gray-200">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Zero Padding</label>
                <Input
                  type="number"
                  min="1"
                  max="10"
            ...
            <p className="text-xs text-gray-500">
              Example: <span className="font-medium">{String(localStartNumber).padStart(localPadding, '0')}</span>
            </p>
          </div>
        )}
        ...
        <label className="flex items-center gap-3 cursor-pointer pt-3 border-t border-gray-200">
          ...
          />
          <span className="text-sm text-gray-700 font-medium">Preserve file extension</span>
        </label>

        {/* Preview */}
        <div className="pt-3 border-t border-gray-200">
          <div className="text-xs text-gray-600 bg-green-50 p-3 rounded-lg border border-green-200">
            <div className="font-medium text-green-800 mb-1">Preview (first 3 files):</div>
            <div className="space-y-0.5 text-green-700">
              <div>1. {getPreview().replace(/\.[^.]+$/, '')}{renamePreserveExtension ? '.jpg' : ''}</div>
              <div>2. {getPreview().replace(/\d+/g, (_, p) => String(parseInt(p || '1') + 1).padStart(localPadding, '0'))}</div>
              <div>3. {getPreview().replace(/\d+/g, (_, p) => String(parseInt(p || '1') + 2).padStart(localPadding, '0'))}</div>
            </div>
          </div>
        </div>
      </div>
