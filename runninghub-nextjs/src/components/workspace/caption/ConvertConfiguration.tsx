...
const OUTPUT_FORMATS = [
  { value: 'jpg', label: 'JPG', extension: '.jpg', description: 'Best for photos' },
  { value: 'png', label: 'PNG', extension: '.png', description: 'Lossless, transparency' },
  { value: 'webp', label: 'WebP', extension: '.webp', description: 'Modern, efficient' },
  { value: 'avif', label: 'AVIF', extension: '.avif', description: 'Next-gen compression' },
];
...
export function ConvertConfiguration({
  ...
  }, [convertOutputFormat, convertQuality, convertLossless, convertOutputSuffix, convertDeleteOriginal, onConfigChange]);
  ...
  const selectedFormat = OUTPUT_FORMATS.find((f) => f.value === convertOutputFormat);
  const supportsQuality = ['jpg', 'webp', 'avif'].includes(convertOutputFormat);

  const getOutputName = (inputName = 'image') => {
    const base = inputName.replace(/\.[^.]+$/, '');
    const suffix = localSuffix || (convertOutputSuffix !== '' ? localSuffix : '');
    ...
    <ConfigurationCard
      ...
      subtitle={
        <>
          ...
          {supportsQuality && !convertLossless && ` • Quality: ${localQuality}%`}
        </>
      }
    >
      <div className="space-y-5">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
            {OUTPUT_FORMATS.map((format) => (
              ...
              >
                ...
                <span className={cn('text-xs', convertOutputFormat === format.value ? 'text-orange-100' : 'text-gray-500')}>
                  {format.description}
                </span>
              </button>
            ))}
        ...
        {supportsQuality && (
          <div className="space-y-3 pt-3 border-t border-gray-200">
            <div className="flex items-center justify-between">
              ...
              <span className="text-sm text-orange-600 font-medium">{localQuality}%</span>
            </div>
            ...
            {['webp', 'avif'].includes(convertOutputFormat) && (
              <label className="flex items-center gap-3 cursor-pointer mt-2">
                ...
                <div className="flex flex-col">
                  <span className="text-sm text-gray-700 font-medium">Lossless compression</span>
                  <span className="text-xs text-gray-500">Larger file, no quality loss</span>
                </div>
              </label>
            )}
          </div>
        ...
        {/* Output Suffix */}
        <div className="space-y-3 pt-3 border-t border-gray-200">
          <label className="block text-xs font-medium text-gray-700 mb-1">Output Suffix (Optional)</label>
          <Input
            type="text"
            value={localSuffix}
        ...
        <label className="flex items-center gap-3 cursor-pointer pt-3 border-t border-gray-200">
          ...
          />
          <div className="flex flex-col">
            <span className="text-sm text-gray-700 font-medium">Delete original after conversion</span>
            <span className="text-xs text-gray-500">The original files will be permanently deleted</span>
          </div>
        </label>
          ...
          <div className="flex items-start gap-2 text-xs text-gray-600 bg-orange-50 p-3 rounded-lg border border-orange-200">
            ...
            <div className="space-y-1">
              ...
              <ul className="list-disc list-inside space-y-0.5 text-orange-700">
                <li><strong>JPG:</strong> Best for photos, small file size, no transparency</li>
                <li><strong>PNG:</strong> Lossless, supports transparency, larger files</li>
                <li><strong>WebP:</strong> Modern format, excellent compression, broad support</li>
                <li><strong>AVIF:</strong> Next-gen format, best compression, newer browsers</li>
              </ul>
            </div>
          </div>
        </div>
