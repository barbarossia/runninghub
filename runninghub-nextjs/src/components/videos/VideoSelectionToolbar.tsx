...
export function VideoSelectionToolbar({
	...
	const handleConvert = useCallback(async () => {
		...
		try {
			...
			toast.error(
				error instanceof Error ? error.message : "Failed to convert videos",
			);
		} finally {
	...
	const handleCrop = useCallback(async () => {
		...
		try {
			...
			toast.error(
				error instanceof Error ? error.message : "Failed to crop videos",
			);
		} finally {
			setIsProcessing(false);
		}
								...
								{onConvert && (
									...
									>
										{isProcessing ? (
											...
											<Video className="h-3.5 w-3.5 mr-1.5 fill-current" />
										)}
