...
export function DuckDecodeDialog({
  ...
  const handleDecode = async () => {
    ...
    try {
      ...
      if (failedDecodes.length > 0) {
        setError(`${failedDecodes.length} image(s) failed to decode:\n${failedDecodes.slice(0, 3).join('\n')}${failedDecodes.length > 3 ? '\n...' : ''}`);
      }
  ...
  const imageLabel = imageCount === 1 ? images[0]?.name : `${imageCount} images`;
        ...
        <div className="space-y-4">
          ...
          <Alert>
            ...
            <AlertDescription className="text-sm">
              This will attempt to decode hidden data from the image(s) using LSB steganography.
            </AlertDescription>
          </Alert>
          ...
          <div>
            ...
            <Input
              ...
              disabled={isDecoding}
              autoComplete="off"
              onKeyDown={(e) => e.key === 'Enter' && !isDecoding && handleDecode()}
            />
          ...
          {isDecoding && (
            <div className="flex items-center gap-2 text-sm">
              ...
              <span>Decoding {imageCount} image{imageCount !== 1 ? 's' : ''}...</span>
            </div>
          )}
            ...
            <Alert>
              ...
              <AlertDescription className="text-sm">
                <div className="font-medium text-green-700">
                  Successfully decoded {results.length} image{results.length !== 1 ? 's' : ''}!
                </div>
              </AlertDescription>
            </Alert>
          )}
