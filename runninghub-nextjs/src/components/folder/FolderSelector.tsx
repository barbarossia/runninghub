...
export default function FolderSelector({
	...
	const handleFolderPicker = async () => {
		...
		try {
			...
			const { images, videos, folders, cleanup } = convertToProcessableFormat(
			...
			if (response.success) {
				console.log(
					"[FolderSelector] Folder selected via picker:",
					response.actual_path || response.virtual_path,
				);
	...
	const handleManualInput = async (pathOverride?: string) => {
		...
		try {
			...
			if (response.success) {
				console.log(
					"[FolderSelector] Folder selected via manual input:",
					...
					{fileSystemStatus.available && (
						<div className="space-y-3">
							...
							<p className="text-xs text-muted-foreground">
								Use your browser&apos;s modern folder picker for secure,
								cross-platform folder access.
							</p>
						</div>
					)}

					{fileSystemStatus.available && !fileSystemStatus.secureContext && (
						<Separator />
					)}
							...
							<AlertDescription className="text-sm">
								<div className="space-y-2">
									...
									<ul className="list-disc list-inside space-y-1 text-xs">
										{getFallbackInstructions().instructions.map(
											(instruction, index) => (
