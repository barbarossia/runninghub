		...
		activeSeriesId: null,
			...
			updateTextContent: (fileId, content, language) =>
				set((state) => {
					...
					const updatedFiles = state.uploadedFiles.map((f) =>
						f.id === fileId ? { ...f, textContent: updated } : f,
					);
			...
			setTextContent: (fileId, content) =>
				set((state) => {
					...
					const updatedFiles = state.uploadedFiles.map((f) =>
						f.id === fileId ? { ...f, textContent: content } : f,
					);
					...
					const sorted = [...files].sort((a, b) => {
						...
						switch (state.mediaSortField) {
							...
							case "date":
								...
								const aDate =
									(a as any).modified_at || (a as any).created_at || 0;
								const bDate =
									(b as any).modified_at || (b as any).created_at || 0;
						...
						return state.mediaSortDirection === "asc"
							? comparison
							: -comparison;
					});
			...
			mergeMediaFiles: (files) =>
				set((state) => {
					const existingById = new Map(
						state.mediaFiles.map((file) => [file.id, file]),
					);
			...
			upsertMediaFile: (file) =>
				set((state) => {
					const existingIndex = state.mediaFiles.findIndex(
					...
					const updatedJobFiles = state.jobFiles.map((jf) => {
						if (jf.filePath === fileId) {
							...
							if (updates.height !== undefined)
								jobUpdates.height = updates.height;
			...
			// NEW ACTIONS - Job Preparation
			// ============================================================
			setJobFiles: (assignments) => set({ jobFiles: assignments, error: null }),

			assignFileToParameter: (filePath, parameterId, mediaFile) =>
				set((state) => {
					// DEBUG: Log mediaFile dimensions
					console.log(
						`[assignFileToParameter] Assigning ${mediaFile.name}: width=${mediaFile.width}, height=${mediaFile.height}`,
					);

					// Remove existing assignment for this file
					const filtered = state.jobFiles.filter(
						(jf) => jf.filePath !== filePath,
					);
					...
					console.log(
						`[assignFileToParameter] Created assignment:`,
						assignment,
					);
			...
			clearJobInputs: () => set({ jobFiles: [] }),

			resetJobPreparation: () => set({ jobFiles: [] }),
				...
				const currentAssignments = state.jobFiles.filter(
					(assignment) =>
						!workflow.inputs.some(
							(input) => input.id === assignment.parameterId,
						),
				);
	...
	deleteJobs: async (jobIds) => {
		...
		return {
			...
			getRecentJobsForWorkflow: (workflowId, limit = 10) => {
				const jobs = get()
					.jobs.filter((j) => j.workflowId === workflowId)
			...
			removeDatasetFile: (fileId) =>
				set((state) => ({
					datasetMediaFiles: state.datasetMediaFiles.filter(
						(f) => f.id !== fileId,
					),
				})),
			...
			onRehydrateStorage: () => (state) => {
				if (state) {
					...
					state.textContents = new Map(
						state.textContents as unknown as Array<
							[string, WorkspaceTextContent]
						>,
					);
					// Convert selectedFiles back to Set
					state.selectedFiles = new Set(
						state.selectedFiles as unknown as string[],
					);
				}
			},
		},
