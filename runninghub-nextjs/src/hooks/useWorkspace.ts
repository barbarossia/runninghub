...
import { API_ENDPOINTS, ENVIRONMENT_VARIABLES, ERROR_MESSAGES } from '@/constants';
import type {
  FileUploadRequest,
  FileUploadResponse,
  ProcessRequest,
  ...
  const uploadImages = useCallback(
    async (files: File[]): Promise<FileUploadResponse[]> => {
      ...
      try {
        ...
        const error = err instanceof Error ? err.message : ERROR_MESSAGES.UPLOAD_FAILED;
        onError?.(error);
        throw err;
      } finally {
        setProcessing(false);
  ...
  const processImages = useCallback(
    async (fileIds: string[]): Promise<ProcessResponse> => {
      ...
      try {
        ...
        const files = useWorkspaceStore.getState().uploadedFiles.filter((f) =>
  ...
  const saveTextContent = useCallback(
    async (
      ...
      try {
        ...
        const error = err instanceof Error ? err.message : ERROR_MESSAGES.SAVE_FAILED;
        onError?.(error);
        return {
          success: false,
          savedPath: '',
