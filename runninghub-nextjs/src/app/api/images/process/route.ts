...
async function processSingleImageWithRetry(
	...
	while (true) {
		// Infinite retry for TASK_QUEUE_MAXED
		...
		const isQueueMaxed =
			result.stdout?.includes("TASK_QUEUE_MAXED") ||
		...
		if (isQueueMaxed) {
			...
			console.log(
				`[${imagePath}] Task queue full, retry ${attempt} after ${delay}ms...`,
			);
			await writeLog(
				`Queue full, retrying ${attempt} for ${imagePath}`,
				"info",
				"process_queue",
			);
...
async function processImagesInBackground(
	...
	await writeLog(
		`Processing ${images.length} images with node ${nodeId}`,
		"info",
		taskId,
	);
	...
	await writeLog(
		`Using workflow: ${workflowId} (CLI will use: ${cliWorkflowId})`,
		"info",
		taskId,
	);
	...
	await writeLog(
		`Concurrency limit: ${maxConcurrent} simultaneous processes`,
		"info",
		taskId,
	);
	...
	try {
		...
		const results = await processWithConcurrency(
			...
			async (imagePath: string, index: number) => {
				...
				const result = await processSingleImageWithRetry(
					imagePath,
					nodeId,
					timeout,
					env,
				...
				if (result.success) {
					await writeLog(
						`✓ Successfully processed: ${imagePath}`,
						"success",
						taskId,
					);
		...
		await writeLog(
			`=== BACKGROUND PROCESSING COMPLETED for task: ${taskId} ===`,
			"info",
			taskId,
		);
		await writeLog(
			`Summary: ${successCount} succeeded, ${failureCount} failed out of ${images.length} total`,
			"info",
			taskId,
		);
