...
function fetchNodesFromCLI(
	...
	return new Promise((resolve, reject) => {
		const python = spawn(
			"python3",
			["-m", "runninghub_cli.cli", "nodes", "--json"],
			{
...
export async function POST(request: NextRequest) {
	try {
		...
		let errorMessage =
			"Invalid workflow ID or failed to fetch workflow details";
		...
		if (error instanceof Error) {
			if (error.message.includes("timed out")) {
				...
				errorMessage =
					"Unable to connect to RunningHub API. Please check your network connection.";
			} else if (
				error.message.includes("401") ||
				error.message.includes("403")
			) {
				errorMessage =
					"Invalid API key. Please check your RunningHub API credentials.";
