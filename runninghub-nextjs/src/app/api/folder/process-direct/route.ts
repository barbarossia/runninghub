...
export async function POST(request: NextRequest) {
	try {
		...
		console.log(
			`Processing ${files.length} items from ${folder_name} via ${source}`,
		);
		...
		if (full_path) {
			...
			try {
				...
				const findFolderInDirectory = async (
					currentPath: string,
					targetName: string,
					currentDepth: number,
					maxDepth: number,
					...
					try {
						...
						for (const entry of entries) {
							if (entry.isDirectory() && !entry.name.startsWith(".")) {
								...
								const found = await findFolderInDirectory(
									subPath,
									targetName,
									currentDepth + 1,
									maxDepth,
				...
				console.log(
					`Searching for folder '${folder_name}' in '${prefixPath}' (max depth: 3)`,
				);
				const foundPath = await findFolderInDirectory(
					prefixPath,
					folder_name,
					0,
					3,
				...
				if (foundPath) {
					...
					console.log(
						`Folder '${folder_name}' not found within depth limit in '${prefixPath}'`,
					);
				}
			} catch (error) {
