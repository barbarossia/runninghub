	...
	try {
		...
		for (let i = 0; i < files.length; i++) {
			...
			if (!file.width || !file.height) {
				console.log(
					`[Resize API] Missing dimensions for ${fileName}, fetching metadata...`,
				);
				...
				if (metadata) {
					...
					console.log(
						`[Resize API] Fetched dimensions: ${file.width}x${file.height}`,
					);
				} else {
					console.warn(
						`[Resize API] Failed to fetch metadata for ${fileName}, proceeding with 0x0 (might skip)`,
					);
				}
			}

			console.log(
				`[Resize API] Processing ${fileName}: ${file.width}x${file.height} -> target: ${longestEdge}`,
			);
			...
			if (
				newDimensions.width === file.width &&
				newDimensions.height === file.height
			) {
				console.log(
					`[Resize API] Skipping ${fileName}: already within target size`,
				);
				results.push({
					fileName,
			...
			if (deleteOriginal) {
				...
				outputPath = join(
					parsedPath,
					`${parsedName}${outputSuffix}${parsedExt}`,
				);
			...
			try {
				console.log(
					`[Resize API] Running resize for ${fileName} -> ${outputPath}`,
				);
				// Perform resize based on file type
				if (file.type === "video") {
					await runFFmpegResize(
						file.path,
						outputPath,
						newDimensions.width,
