	...
	try {
		...
		for (const entry of entries) {
			if (entry.isDirectory()) {
				...
				try {
					const datasetEntries = await readdir(datasetPath, {
						withFileTypes: true,
					});
		...
		if (datasets.length > 0) {
			...
			console.log(
				`Migrated ${datasets.length} existing datasets to workspace.json`,
			);
		}
	} catch (error) {
