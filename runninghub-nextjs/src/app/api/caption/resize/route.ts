...
async function processResizeInBackground(
  ...
  await writeLog(`Mode: ${config.mode}, Delete original: ${config.deleteOriginal}`, 'info', taskId);
  ...
  for (let i = 0; i < images.length; i++) {
    ...
    try {
      ...
      await writeLog(`Failed to resize ${basename(inputPath)}: ${error}`, 'error', taskId);
      failureCount++;
      await updateTask(taskId, { failedCount: failureCount });
    }
  }

  await writeLog(`=== RESIZE COMPLETED: ${successCount}/${images.length} ===`, 'info', taskId);
