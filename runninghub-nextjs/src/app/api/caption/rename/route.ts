...
function generateNewName(
  ...
  const { pattern, prefix, suffix, startNumber, padding, template, preserveExtension } = config;
  ...
  if (pattern === 'prefix-sequence') {
    ...
    newName = template
      ...
      .replace(/\{index:(\d+)d\}/, (_, width) => String(seqNumber).padStart(parseInt(width), '0'))
...
async function processRenameInBackground(
  ...
  await writeLog(`Renaming ${mediaFiles.length} files with pattern: ${config.pattern}`, 'info', taskId);
  ...
  for (let i = 0; i < sortedFiles.length; i++) {
    ...
    await updateTask(taskId, { currentImage: `${basename(oldPath)} (${i + 1}/${sortedFiles.length})` });
    ...
    try {
      ...
      try {
        ...
        await writeLog(`Renamed: ${basename(oldPath)} -> ${newName}`, 'success', taskId);
        successCount++;
      }

      await updateTask(taskId, { completedCount: successCount });
    } catch (error) {
      await writeLog(`Failed to rename ${basename(oldPath)}: ${error}`, 'error', taskId);
      failureCount++;
      await updateTask(taskId, { failedCount: failureCount });
    }
  }

  await writeLog(`=== RENAME COMPLETED: ${successCount}/${sortedFiles.length} ===`, 'info', taskId);
