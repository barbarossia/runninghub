      ...
      return NextResponse.json(
        {
          ...
          message: "Please configure a RunningHub workflow ID for captioning in the settings"
        },
        { status: 400 }
      );
    }
...
async function processCaptionsInBackground(
  ...
  await writeLog(`Caption workflow integration not yet implemented`, 'warning', taskId);
  await writeLog(`Workflow ID "${config.workflowId}" will be used for captioning`, 'info', taskId);
  ...
  for (let i = 0; i < mediaFiles.length; i++) {
    ...
    await updateTask(taskId, { currentImage: `${filePath} (${i + 1}/${mediaFiles.length})` });
    await writeLog(`Placeholder: Would caption ${filePath}`, 'info', taskId);
  }

  await writeLog(`=== CAPTIONING COMPLETED: ${successCount} succeeded, ${failureCount} failed ===`, 'info', taskId);
