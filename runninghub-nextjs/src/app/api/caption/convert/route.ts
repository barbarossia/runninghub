...
async function processConvertInBackground(
  ...
  await writeLog(`Converting ${images.length} images to ${config.outputFormat}`, 'info', taskId);
  ...
  for (let i = 0; i < images.length; i++) {
    ...
    await updateTask(taskId, { currentImage: `${basename(inputPath)} (${i + 1}/${images.length})` });
    ...
    try {
      ...
      if (success) {
        ...
        if (config.deleteOriginal) {
          ...
          await writeLog(`Deleted original: ${basename(inputPath)}`, 'info', taskId);
        }
      ...
      await writeLog(`Failed to convert ${basename(inputPath)}: ${error}`, 'error', taskId);
      failureCount++;
      await updateTask(taskId, { failedCount: failureCount });
    }
  }

  await writeLog(`=== CONVERT COMPLETED: ${successCount}/${images.length} ===`, 'info', taskId);
