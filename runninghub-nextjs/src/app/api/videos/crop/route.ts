...
async function cropVideosInBackground(
	...
	await writeLog(
		`=== VIDEO CROPPING STARTED for task: ${taskId} ===`,
		"info",
		taskId,
	);
	...
	try {
		for (let i = 0; i < videos.length; i++) {
			...
			if (result.success) {
				successCount++;
				await writeLog(`✓ Cropped: ${videoPath}`, "success", taskId);
		...
		await writeLog(
			`=== VIDEO CROPPING COMPLETED: ${successCount} success, ${failureCount} failed ===`,
			"info",
			taskId,
		);
