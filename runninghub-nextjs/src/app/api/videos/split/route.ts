	...
	try {
		...
		if (!ffmpegAvailable) {
			return NextResponse.json(
				{
					error:
						"FFmpeg is not installed or not accessible. Please install FFmpeg to use video splitting features.",
				},
				{ status: 500 },
			);
		}
...
async function splitSingleVideo(
	...
	console.log(
		`Splitting ${videoName}: duration=${duration}s, segmentDuration=${segmentDuration}s, segmentCount=${segmentCount}`,
	);
...
async function splitVideosInBackground(
	...
	await writeLog(
		`Splitting ${videos.length} videos (${modeInfo})`,
		"info",
		taskId,
	);
