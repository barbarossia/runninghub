...
export async function POST(request: NextRequest) {
  try {
        ...
        if (fs.existsSync(jobJsonPath)) {
          ...
          if (jobData.results && jobData.results.outputs) {
            ...
            jobData.results.outputs = jobData.results.outputs.map((output: any) => {
              ...
              if (output.path === duckImagePath || output.workspacePath === duckImagePath || output.fileName === originalFileName) {
                updated = true;
                return {
                  ...output,
                  path: decodedFilePath,
                  fileName: path.basename(decodedFilePath),
                  fileType: path.extname(decodedFilePath).toLowerCase() === '.mp4' ? 'video' : 'image',
                  fileSize: fileSize,
    ...
    return NextResponse.json({
      ...
      decodedFileType: path.extname(decodedFilePath).toLowerCase() === '.mp4' ? 'video' : 'image',
      fileSize,
