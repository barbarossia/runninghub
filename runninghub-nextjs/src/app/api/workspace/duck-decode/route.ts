	...
	try {
		...
		const detectFileType = (
			...
			try {
				...
				if (
					firstBytes.startsWith("52494646") &&
					buffer.slice(8, 12).toString("hex").toLowerCase() === "41564920"
				) {
					return "video";
				}
				...
				if (
					firstBytes.startsWith("52494646") &&
					buffer.slice(8, 12).toString("hex").toLowerCase() === "57454250"
				) {
					return "image";
				}
		...
		const tempOutputPath = path.join(
			resultDir,
			`${originalFileNameWithoutExt}_decoded${originalFileExt}`,
		);
		...
		console.log(
			"[Duck Decode] Command completed, output length:",
			result.length,
		);
		...
		const decodedPathMatch = result.match(
			/Successfully saved extracted data to: (.+)/,
		);
		...
		console.log(
			`[Duck Decode] Extracted decoded file path: ${decodedFilePath}`,
		);

		// Verify the file exists
		if (!fs.existsSync(decodedFilePath)) {
			console.error(
				`[Duck Decode] Decoded file not found at: ${decodedFilePath}`,
			);
		...
		try {
			...
			if (fs.existsSync(duckImagePath)) {
				...
				console.warn(
					`Original duck image not found at ${duckImagePath}, skipping move`,
				);
			}
		} catch (error) {
			console.error(
				"Failed to move original duck image to encoded folder:",
				error,
			);
		...
		try {
			...
			console.log(
				`[Duck Decode] Detected decoded file type: ${decodedFileType}`,
			);
			...
			if (decodedFileType === "video") {
				...
				finalOutputPath = path.join(
					resultDir,
					`${originalFileNameWithoutExt}_decoded.mp4`,
				);
				console.log(
					`[Duck Decode] Video detected, using _decoded.mp4 extension`,
				);
				...
				finalOutputPath = path.join(
					resultDir,
					`${originalFileNameWithoutExt}_decoded${finalExtension}`,
				);
				console.log(
					`[Duck Decode] Image detected, using ${finalExtension} extension with _decoded suffix`,
				);
			} else {
				...
				console.log(
					`[Duck Decode] Unknown file type, keeping original extension with _decoded suffix`,
				);
			}
			...
			if (
				...
				finalOutputPath !== decodedFilePath
			) {
			...
			if (decodedFilePath !== finalOutputPath) {
				...
				console.log(
					`Decoded file is already at final path: ${finalOutputPath}`,
				);
			}
		...
		const isJobOutput =
			path.basename(resultDirBase) === "result" &&
			path.basename(parentOfResult).startsWith("job_");
			...
			try {
				...
				if (fs.existsSync(jobJsonPath)) {
					...
					if (jobData.results && jobData.results.outputs) {
						let updated = false;
						jobData.results.outputs = jobData.results.outputs.map(
							(output: any) => {
								// Match by path or workspacePath
								// We check against duckImagePath (which was renamed to encoded folder)
								// But in job.json it might still be the old path
								if (
									output.path === duckImagePath ||
									output.workspacePath === duckImagePath ||
									output.fileName === originalFileName
								) {
									updated = true;
									return {
										...output,
										...
										fileType:
											path.extname(decodedFilePath).toLowerCase() === ".mp4"
												? "video"
												: "image",
										fileSize: fileSize,
		...
		if (
			errorMessage.includes("Insufficient image data") ||
			errorMessage.includes("图像数据不足")
		) {
			return NextResponse.json(
		...
		if (
			errorMessage.includes("Payload length invalid") ||
			errorMessage.includes("载荷长度异常")
		) {
			return NextResponse.json(
		...
		if (
			errorMessage.includes("Header corrupted") ||
			errorMessage.includes("文件头损坏")
		) {
			return NextResponse.json(
