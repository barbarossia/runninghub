...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!ffmpegAvailable) {
      return NextResponse.json(
        { error: "FFmpeg is not installed or not accessible. Please install FFmpeg to use video conversion features." },
        { status: 500 }
      );
    }
    ...
    convertVideosInBackground(videos, targetFps, deleteOriginal, outputSuffix, crf, preset, timeout, taskId);
      ...
      if (success) {
        try {
          ...
          if (await fileExists(tempOutputPath)) {
            ...
            console.log(`[${videoPath}] Renamed temp file to final output: ${outputPath}`);
          }
...
async function convertVideosInBackground(
  ...
  await writeLog(`Converting ${videos.length} videos to ${targetFps} FPS`, "info", taskId);
