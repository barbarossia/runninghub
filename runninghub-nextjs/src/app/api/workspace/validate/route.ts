...
import { validateFileForParameter, filterValidFilesForParameter } from '@/utils/workspace-validation';
  ...
  try {
    ...
    if (body.file && body.parameter) {
      ...
      if (!file || !parameter) {
        ...
        } as { success: false; error: string }, { status: 400 });
      }
    ...
    return NextResponse.json({
      success: false,
      error: 'Invalid request. Provide either {file, parameter} or {files, parameter}',
    } as { success: false; error: string }, { status: 400 });
    ...
    } as { success: false; error: string }, { status: 500 });
  }
}
