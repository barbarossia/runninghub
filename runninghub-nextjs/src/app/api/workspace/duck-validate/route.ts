...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      const apiKey = process.env.NEXT_PUBLIC_RUNNINGHUB_API_KEY || process.env.RUNNINGHUB_API_KEY;
      const workflowId = process.env.NEXT_PUBLIC_RUNNINGHUB_WORKFLOW_ID || process.env.RUNNINGHUB_WORKFLOW_ID;
      ...
      console.log('[Duck Validate] Validation succeeded (no password required)');
      ...
      console.error('[Duck Validate] Error message:', errorMessage.slice(0, 200));
      ...
      if (fullError.includes('Wrong password') || fullError.includes('密码错误')) {
      ...
      if (fullError.includes('Insufficient image data') ||
