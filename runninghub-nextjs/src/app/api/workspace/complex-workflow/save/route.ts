...
export async function POST(request: NextRequest) {
  try {
    ...
    // Validate request
    if (!body.workflow || !body.workflow.name || !body.workflow.steps || body.workflow.steps.length === 0) {
      return NextResponse.json({
        success: false,
        error: 'Invalid complex workflow data',
      } as SaveComplexWorkflowResponse, { status: 400 });
    }
    ...
    return NextResponse.json({
      success: false,
      error: error instanceof Error ? error.message : 'Failed to save complex workflow',
    } as SaveComplexWorkflowResponse, { status: 500 });
  }
}
