...
import { saveComplexWorkflowExecution, mapOutputsToInputs } from '@/lib/complex-workflow-utils';
import type {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const firstStep = complexWorkflow.steps.find((s: WorkflowStep) => s.stepNumber === 1);
    if (!firstStep) {
      return NextResponse.json({
        success: false,
    ...
    const updatedExecution = { ...execution, steps: execution.steps };
    ...
    return NextResponse.json({
      success: false,
      error: error instanceof Error ? error.message : 'Failed to execute complex workflow',
    } as ExecuteComplexWorkflowResponse, { status: 500 });
  }
}
