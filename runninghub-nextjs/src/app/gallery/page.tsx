...
import { FolderOpen, AlertCircle, Images, Settings } from "lucide-react";
...
export default function GalleryPage() {
	...
	const { deselectAll, selectedImages } = useSelectionStore();

	// Use legacy stores for process config (until migrated)
	const { config: processConfig, setConfig: setProcessConfig } =
		useProcessStore();
	...
	useEffect(() => {
		...
		console.log(
			`[Gallery] Starting duck validation for ${imagesToValidate.length} images...`,
		);
	...
	const handleDelete = async (selectedPaths: string[]) => {
		try {
			...
			deselectAll();
			logger.success(
				`Deleted ${selectedPaths.length} image${selectedPaths.length !== 1 ? "s" : ""}`,
				{
					metadata: {
						count: selectedPaths.length,
						paths: selectedPaths.slice(0, 3),
					},
				},
			);
		} catch (err) {
	...
	const handleExport = async (selectedPaths: string[]) => {
		...
		try {
			...
			const errorMessage =
				err instanceof Error ? err.message : "Failed to export images";
	...
	const handleSortChange = useCallback(
		(field: typeof sortField, direction: typeof sortDirection) => {
				...
				{!selectedFolder ? (
					...
					<div className="space-y-6">
						...
						<SelectionToolbar
							...
							showComplexWorkflowButton={true}
							selectedImagePaths={Array.from(selectedImages.keys())}
						/>
						...
						{isLoadingFolder || isLoadingImages ? (
							<ImageGallerySkeleton
								viewMode={useImageStore.getState().viewMode}
								count={12}
							/>
						) : (
							<ImageGallery
